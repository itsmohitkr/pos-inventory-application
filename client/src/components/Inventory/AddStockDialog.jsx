...
const AddStockDialog = ({ open, onClose, product, onStockAdded }) => {
  ...
  const handleChange = (name, value) => {
    ...
    if (name === 'mrp' || name === 'selling_price') {
      ...
      const s =
        name === 'selling_price' ? parseFloat(value) : parseFloat(stockData.selling_price || 0);
      if (m > 0) {
        setDiscountInput((((m - s) / m) * 100).toFixed(1));
      } else {
        setDiscountInput('0');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        wholesalePrice: stockData.wholesaleEnabled
          ? parseFloat(stockData.wholesalePrice) || 0
          : null,
        wholesaleMinQty: stockData.wholesaleEnabled
          ? parseInt(stockData.wholesaleMinQty) || 0
          : null,
