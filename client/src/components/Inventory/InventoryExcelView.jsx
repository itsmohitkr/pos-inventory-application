...
import {
    ...
    Slide, Container, Chip, Popover, Checkbox, FormControlLabel,
    FormGroup
} from '@mui/material';
import {
    Close as CloseIcon,
...
const InventoryExcelView = ({ open, onClose, categoryFilter = 'all', externalSearch = '' }) => {
    ...
    const [cols, setCols] = useState({
        sno: true, name: true,
        stockStatus: true, batchCode: true, category: true,
        mrp: true, sp: true, cp: true, profitRs: true,
    ...
    const flatData = useMemo(() => {
        ...
        const filteredProducts = localCategoryFilter === 'all'
            ? products
    ...
    const totals = useMemo(() => {
        ...
        return {
            ...
            avgDiscPctVendor: totalValueMrp > 0 ? ((totalValueMrp - totalValueCost) / totalValueMrp) * 100 : 0,
            avgDiscRsCust: totalStock > 0 ? (totalValueMrp - totalValueSelling) / totalStock : 0,
            avgDiscPctCust: totalValueMrp > 0 ? ((totalValueMrp - totalValueSelling) / totalValueMrp) * 100 : 0,
    ...
    const handleExportCSV = () => {
        ...
        const csvContent = [
            ...
            ...filteredAndSortedData.map((row, idx) => {
                ...
                if (cols.createdAt) rowData.push(row.createdAt !== 'N/A' ? new Date(row.createdAt).toLocaleDateString() : 'N/A');
                return rowData.join(',');
            })
        ].join('\n');
    ...
    return (
        ...
        >
                ...
                <Toolbar variant="dense">
                    ...
                    >
                        <Box sx={{ p: 2, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, minWidth: 400 }}>
                            <Typography variant="subtitle2" sx={{ gridColumn: '1 / -1', mb: 1, fontWeight: 'bold' }}>Select Columns to Display</Typography>
            ...
            <Container maxWidth={false} sx={{ py: 3, bgcolor: '#f8f9fa', minHeight: 'calc(100vh - 48px)' }}>
                ...
                <TableContainer component={Paper} sx={{ maxHeight: 'calc(100vh - 250px)', borderRadius: 2, border: '1px solid #ccc' }}>
                    <Table stickyHeader size="small" sx={{
                        ...
                        <TableHead>
                            <TableRow>
                                {cols.sno && (
                                    <TableCell sx={{
                                        bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap',
                                        position: 'sticky', left: 0, zIndex: 3, borderRight: '2px solid #ccc'
                                    }}>S.No</TableCell>
                                )}
                                {cols.name && (
                                    <TableCell sx={{
                                        bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap',
                                        position: 'sticky', left: cols.sno ? 45 : 0, zIndex: 3, borderRight: '2px solid #ccc'
                                    }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'name')} direction={sortConfigs.find(c => c.key === 'name')?.direction || 'asc'} onClick={(e) => handleSort('name', e)}>Name</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.stockStatus && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'stockStatus')} direction={sortConfigs.find(c => c.key === 'stockStatus')?.direction || 'asc'} onClick={(e) => handleSort('stockStatus', e)}>Status</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.batchCode && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'batchCode')} direction={sortConfigs.find(c => c.key === 'batchCode')?.direction || 'asc'} onClick={(e) => handleSort('batchCode', e)}>Batch Code</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.category && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'category')} direction={sortConfigs.find(c => c.key === 'category')?.direction || 'asc'} onClick={(e) => handleSort('category', e)}>Category</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.mrp && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'mrp')} direction={sortConfigs.find(c => c.key === 'mrp')?.direction || 'asc'} onClick={(e) => handleSort('mrp', e)}>MRP</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.sp && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'sp')} direction={sortConfigs.find(c => c.key === 'sp')?.direction || 'asc'} onClick={(e) => handleSort('sp', e)}>Selling Price</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.cp && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'cp')} direction={sortConfigs.find(c => c.key === 'cp')?.direction || 'asc'} onClick={(e) => handleSort('cp', e)}>Cost Price</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.profitRs && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'profitRs')} direction={sortConfigs.find(c => c.key === 'profitRs')?.direction || 'asc'} onClick={(e) => handleSort('profitRs', e)}>Profit Unit(Rs)</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.discRsVendor && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'discRsVendor')} direction={sortConfigs.find(c => c.key === 'discRsVendor')?.direction || 'asc'} onClick={(e) => handleSort('discRsVendor', e)}>Disc Vendor(Rs)</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.discPctVendor && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'discPctVendor')} direction={sortConfigs.find(c => c.key === 'discPctVendor')?.direction || 'asc'} onClick={(e) => handleSort('discPctVendor', e)}>Disc Vendor(%)</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.discRsCust && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'discRsCust')} direction={sortConfigs.find(c => c.key === 'discRsCust')?.direction || 'asc'} onClick={(e) => handleSort('discRsCust', e)}>Disc Cust(Rs)</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.discPctCust && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'discPctCust')} direction={sortConfigs.find(c => c.key === 'discPctCust')?.direction || 'asc'} onClick={(e) => handleSort('discPctCust', e)}>Disc Cust(%)</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.marginPct && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'marginPct')} direction={sortConfigs.find(c => c.key === 'marginPct')?.direction || 'asc'} onClick={(e) => handleSort('marginPct', e)}>Margin (%)</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.barcode && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'barcode')} direction={sortConfigs.find(c => c.key === 'barcode')?.direction || 'asc'} onClick={(e) => handleSort('barcode', e)}>Barcode</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.expiry && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'expiry')} direction={sortConfigs.find(c => c.key === 'expiry')?.direction || 'asc'} onClick={(e) => handleSort('expiry', e)}>Expiry</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.wsPrice && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'wsPrice')} direction={sortConfigs.find(c => c.key === 'wsPrice')?.direction || 'asc'} onClick={(e) => handleSort('wsPrice', e)}>WS Price</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.wsMinQty && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                ...
                                {cols.totalValCp && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'totalValCp')} direction={sortConfigs.find(c => c.key === 'totalValCp')?.direction || 'asc'} onClick={(e) => handleSort('totalValCp', e)}>Total Value (Cost)</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.totalValSp && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'totalValSp')} direction={sortConfigs.find(c => c.key === 'totalValSp')?.direction || 'asc'} onClick={(e) => handleSort('totalValSp', e)}>Total Rev (Selling)</TableSortLabel>
                                    </TableCell>
                                )}
                                {cols.createdAt && (
                                    <TableCell sx={{ bgcolor: '#e8eaf6', fontWeight: 800, whiteSpace: 'nowrap' }}>
                                        <TableSortLabel active={sortConfigs.some(c => c.key === 'createdAt')} direction={sortConfigs.find(c => c.key === 'createdAt')?.direction || 'asc'} onClick={(e) => handleSort('createdAt', e)}>Added On</TableSortLabel>
                                    </TableCell>
                                )}
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {filteredAndSortedData.map((row, index) => (
                                ...
                                >
                                    {cols.sno && <TableCell className="sticky-col" sx={{
                                        fontWeight: 500, color: 'text.secondary', bgcolor: 'background.paper',
                                        position: 'sticky', left: 0, zIndex: 2, borderRight: '2px solid #ccc',
                                        transition: 'background-color 0.2s',
                                        '.MuiTableRow-hover:hover &': { bgcolor: '#f5f5f5' }
                                    }}>{index + 1}</TableCell>}
                                    {cols.name && <TableCell className="sticky-col" sx={{
                                        fontWeight: 700, bgcolor: 'background.paper',
                                        position: 'sticky', left: cols.sno ? 45 : 0, zIndex: 2, borderRight: '2px solid #ccc',
                                        transition: 'background-color 0.2s',
                                        '.MuiTableRow-hover:hover &': { bgcolor: '#f5f5f5' }
                                    }}>{row.name}</TableCell>}
                                    {cols.stockStatus && (
                                        <TableCell sx={{
                                            fontWeight: 600,
                                            color: row.stockStatus === 'In Stock' ? 'success.main' : row.stockStatus === 'Low Stock' ? 'warning.main' : 'error.main'
                                        }}>
                                            {row.stockStatus}
                                        </TableCell>
                                    )}
                                    ...
                                    {cols.profitRs && <TableCell sx={{ py: 0.5, fontWeight: 700, color: row.profitRs > 0 ? 'success.main' : 'error.main' }}>{row.profitRs.toFixed(2)}</TableCell>}
                        ...
                        <TableBody>
                            <TableRow sx={{
                                ...
                                <TableCell colSpan={cols.sno ? 2 : 1} sx={{
                                    fontWeight: 800, py: 1.5,
                                    position: 'sticky', left: 0, zIndex: 5, borderRight: '2px solid #ccc'
                                }}>TOTALS / AVERAGES</TableCell>
