...
const Receipt = ({ sale, settings, shopMetadata }) => {
    ...
    // Map paper sizes to safe printable widths for thermal printers
    // 80mm ribbon usually has 72mm-76mm printable area.
    // 72mm ribbon usually has 64mm-68mm printable area.
    const getSafeWidth = (size) => {
        if (size === '80mm') return '72mm'; // Most 80mm printers have 72mm printable
        if (size === '72mm') return '64mm'; // User says 72mm works, but let's be safe
                                    ...
                                    {config.productName && (
                                        <td style={{ padding: '6px 0', verticalAlign: 'top', maxWidth: '120px' }}>
                                            <div style={{
                                                ...
                                                {item.isFree && <span style={{ fontSize: '0.8em', marginLeft: '4px', color: '#000' }}>(Free)</span>}
                                            </div>
