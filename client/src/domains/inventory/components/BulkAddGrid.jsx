...
const BulkAddGrid = ({ onProductsAdded, onCancel }) => {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await inventoryService.bulkUpdate(products);
  ...
  const handleClear = async () => {
    const confirmed = await showConfirm(
      'Are you sure you want to clear all rows? This action cannot be undone.'
    );
    if (confirmed) {
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField {...params} fullWidth size="small" placeholder="Category path" />
                    )}
                  />
                </TableCell>
                <TableCell>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', py: 1 }}>
        <Button onClick={onCancel} variant="text" color="inherit" startIcon={<ArrowBackIcon />}>
          Back to Inventory
        </Button>
      </Box>
