...
/**
 * Report payload from GET /api/reports. Typed loosely for now — the server
 * assembles it from several aggregations and the exact shape is only knowable
 * once server/src/domains/report is converted.
 * TODO(ts-migration): replace with the server's return type.
 */
type ReportPayload = Record<string, any>; // eslint-disable-line @typescript-eslint/no-explicit-any

/** One entry from GET /api/reports/monthly (12 fixed buckets). */
...
/** One entry from GET /api/reports/daily (one per day of month). */
...
export type HourlyMetric = 'amount' | 'count';
...
export const useDashboardData = () => {
  ...
  const [hourlyMetric, setHourlyMetric] = useState<HourlyMetric>('amount');
  ...
  const fetchPeriodicData = useCallback(async (start: string, end: string) => {
    setLoading(true);
    try {
