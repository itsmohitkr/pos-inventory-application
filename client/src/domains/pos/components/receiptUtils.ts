...
export const getSafePrintableWidth = (size?: string): string => {
...
export const getReceiptTheme = (billFormat?: string) => {
  return RECEIPT_THEMES[billFormat as keyof typeof RECEIPT_THEMES] || RECEIPT_THEMES.Standard;
};

export const getReceiptCalculations = (sale: ReceiptSale, config: ReceiptSettings) => {
  ...
  const totalMrp =
    sale.items?.reduce((sum: number, item: ReceiptSaleItem) => {
  ...
  const totalItemCount = sale.items?.reduce((sum: number, item: ReceiptSaleItem) => sum + (item.quantity || 1), 0) || 0;
