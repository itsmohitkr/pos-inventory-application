...
/**
 * Add/edit user form. `status` is only present in edit mode — the add form
 * omits it and the server applies its default.
 */
    ...
    <>
      <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                          {visiblePasswords[user.id] ? (user as { password?: string }).password : '••••••••'}
                        </Typography>
        ...
        >
          ...
          <AddUserContent sx={{ minWidth: 400 }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
              <TextField label="Username" value={formData.username} disabled fullWidth />
              <TextField
                select
                label="Role"
                value={formData.role}
