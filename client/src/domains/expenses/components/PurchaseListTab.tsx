...
import {
  ...
  Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TableFooter, Chip, Autocomplete, IconButton, InputAdornment, Menu, Popover, Tooltip, Badge,
} from '@mui/material';
import {
  Add as AddIcon,
...
const PurchaseListTab = ({
  ...
  const [activePurchase, setActivePurchase] = React.useState<Purchase | null>(null);
  const [filterAnchorEl, setFilterAnchorEl] = React.useState<null | HTMLElement>(null);
  const hasActiveFilters = purchaseStatusFilter !== 'All' || purchaseVendorFilter !== 'All';
  ...
  return (
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'nowrap' }}>
          <TextField
            ...
            InputProps={{
              ...
              endAdornment: purchaseSearchFilter ? (
                <InputAdornment position="end">
                  <IconButton size="small" onClick={() => setPurchaseSearchFilter('')} edge="end">
    ...
    {/* Filter Popover for Vendor & Status */}
    <Popover
    ...
    >
      ...
      <Stack spacing={2}>
        ...
        <Box>
          ...
          >
            <MenuItem value="All" sx={{ fontWeight: 600, fontSize: '0.82rem' }}>All Statuses</MenuItem>
            <MenuItem value="Paid" sx={{ fontWeight: 500, fontSize: '0.82rem' }}>Paid</MenuItem>
            <MenuItem value="Due" sx={{ fontWeight: 500, fontSize: '0.82rem' }}>Due</MenuItem>
            <MenuItem value="Unpaid" sx={{ fontWeight: 500, fontSize: '0.82rem' }}>Unpaid</MenuItem>
          </TextField>
        </Box>
      </Stack>
    </Popover>
      ...
      <Table size="small" stickyHeader sx={{ width: '100%' }}>
        ...
        <TableBody>
          {filteredPurchases.map((row) => {
            const isSelected = selectedPurchase?.id === row.id;

            return (
              <TableRow
                key={row.id}
                onClick={() => onSelectPurchase?.(isSelected ? null : row)}
                hover
                sx={{
                  cursor: 'pointer',
              ...
              >
                ...
                <TableCell sx={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', px: 1.5, py: 1 }}>
                  <Typography variant="body2" sx={columnTypographySx}>
                    {row.note || '—'}
                  </Typography>
                </TableCell>
          ...
          {filteredPurchases.length === 0 && (
            <TableRow>
              <TableCell colSpan={8} align="center" sx={{ py: 8 }}>
                <Box sx={{ opacity: 0.6 }}>
                  <Typography variant="body2" sx={{ color: '#64748b', fontWeight: 800 }}>NO PURCHASES FOUND</Typography>
    ...
    >
      {onOpenPaymentDialog && activePurchase && (activePurchase.dueAmount || 0) > 0 && (
        <MenuItem
          onClick={(e) => {
            e.stopPropagation();
            onOpenPaymentDialog(activePurchase);
            handleCloseMenu();
          }}
      ...
      <MenuItem
        onClick={(e) => {
          e.stopPropagation();
          if (activePurchase) {
            onDeletePurchase(activePurchase.id);
          }
          handleCloseMenu();
        }}
