...
router
  .route('/login')
  .post(validateRequest({ body: loginBodySchema }), asyncHandler(authController.login))
  .all(methodNotAllowed);
router
  .route('/profile')
  .get(validateRequest({ query: profileQuerySchema }), asyncHandler(authController.getProfile))
  .all(methodNotAllowed);
router
  .route('/users')
  .get(asyncHandler(authController.getAllUsers))
  .post(validateRequest({ body: createUserBodySchema }), asyncHandler(authController.createUser))
  .all(methodNotAllowed);
router
  .route('/users/:id')
  .put(
  ...
  .delete(validateRequest({ params: userIdParamSchema }), asyncHandler(authController.deleteUser))
  .all(methodNotAllowed);
router
...
router
  ...
  .post(
    ...
    asyncHandler(authController.wipeDatabase)
  )
  .all(methodNotAllowed);
router
  .route('/verify-admin')
