...
// Payment endpoints
router.route('/:id/payments').post(validateRequest({ params: expenseIdParamSchema, body: paymentBodySchema }), asyncHandler(expenseController.addPayment)).all(methodNotAllowed);
router.route('/payments/:id').put(validateRequest({ params: expenseIdParamSchema, body: paymentBodySchema }), asyncHandler(expenseController.updatePayment)).delete(validateRequest({ params: expenseIdParamSchema }), asyncHandler(expenseController.deletePayment)).all(methodNotAllowed);
