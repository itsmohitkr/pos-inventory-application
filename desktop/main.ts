...
interface PrintManualRequest {
   ...
   * a big page instead of filling the roll width. The renderer computes
   * this from the shop's configured paperSize preset (see
   * client/src/shared/utils/receiptPrintOptions.ts) rather than a measured
   * custom value — an earlier attempt using a computed custom width+height
   * pair regressed real hardware, likely because the driver didn't
   * recognize the arbitrary size as one of its supported presets.
   */
